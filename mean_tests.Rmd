...
data %>%
  select(pdi, treatment) %>%
  car::leveneTest(data = ., pdi ~ treatment)
```
...
There is no evidence that mdi is not normal (p = 0.13), and it is visually close to a normal distribution.
...
data %>%
  select(mdi, treatment) %>%
  car::leveneTest(data = ., mdi ~ treatment)
```

There is no evidence of no different variances (p = 0.46).
...
This t-test considers two different variances. There is no evidence that the distr. means are different (p = 0.21).
